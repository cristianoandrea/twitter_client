...
def dictify_place(place):
    if place != None:
        place_dict={}
        posti_long1=(place.bounding_box.coordinates[0][0][0] + place.bounding_box.coordinates[0][1][0])/2
        posti_lat1=(place.bounding_box.coordinates[0][0][1]+place.bounding_box.coordinates[0][1][1])/2
        posti_long2=(place.bounding_box.coordinates[0][2][0]+place.bounding_box.coordinates[0][3][0])/2
        posti_lat2=(place.bounding_box.coordinates[0][2][1]+place.bounding_box.coordinates[0][3][1])/2
        posti_long=(posti_long1+posti_long2)/2
        posti_lat=(posti_lat1+posti_lat2)/2
        round(posti_lat,2)
        round(posti_long, 2)
        diff_lat=randrange(-9,9)
        diff_long=randrange(-9,9)
        posti_lat=posti_lat+(diff_lat/10000)
        posti_long=posti_long+(diff_long/10000)
...
def dictify_single_tweet(tweet):
    ...
    tweet_dict['place'] = dictify_place(tweet.place)
    if is_retweet(tweet):
